...
jQuery(document).ready(function($) {
    ...
    if (!$('link[href*="elifin-ai-frontend.css"]').length) {
        // IMPORTANT: Use the correct path based on your deployment structure if this fails.
    ...
    function generateWidgetHtml() {
        // CRITICAL FIX: Using inline style for fixed positioning and z-index (99999) 
        // to guarantee visibility and placement, regardless of other conflicting CSS or Tailwind loading issues.
        const widgetHtml = `
            ...
            <div id="chat-widget-container" style="position: fixed; bottom: 16px; right: 16px; z-index: 99999;">
                ...
                <div id="chat-window"
                    ...
                    <!-- Header -->
                    <div class="flex items-center justify-between p-4 bg-zinc-800 text-white shadow-md">
    ...
    const restartButton = $('#restart-chat-button'); // New reference
    ...
    function sendMessage() {
        ...
        // Reading a DOM property like offsetWidth ensures the browser calculates layout, 
        // which reliably triggers CSS animations on mobile devices where they might otherwise be deferred.
        // We use vanilla JS access ([0]) for efficiency in this critical, synchronous step.
        ...
        function handleSuccessfulResponse(data, messageId) {
            let assistantResponseText = 'Sorry, I received an empty response.';
            let errorText = null;
            ...
            if (data && data.text) {
                assistantResponseText = data.text;
                 ...
                 errorText = `AI Service Error: ${data.error}. Check plugin settings.`;
                 assistantResponseText = errorText;
            } else {
                assistantResponseText = 'Received unexpected data format.';
            }
    ...
    emojiPicker.find('span').each(function() {
        $(this).addClass('cursor-poxinter text-xl p-1 rounded-md hover:bg-zinc-600 transition');
