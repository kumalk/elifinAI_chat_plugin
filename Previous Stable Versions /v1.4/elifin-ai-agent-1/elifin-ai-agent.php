...
function cbfa_register_settings() {
    ...
    register_setting('cbfa_options_group', 'cbfa_quick_links_list'); 
    ...
    // NEW: Typing Delay Setting
    register_setting('cbfa_options_group', 'cbfa_typing_delay_ms_per_char', array('sanitize_callback' => 'intval'));
    ...
    add_settings_field('cbfa_quick_links_list_field', 'Quick Action Links (JSON)', 'cbfa_quick_links_list_callback', 'chatbase-assistant-config', 'cbfa_config_section'); // <-- NEW FIELD
    ...
    // New Typing Delay Field
    add_settings_field('cbfa_typing_delay_field', 'Typing Speed (ms/char)', 'cbfa_typing_delay_ms_per_char_callback', 'chatbase-assistant-styling', 'cbfa_style_section');
...
function cbfa_enqueue_admin_assets($hook) {
    ...
    wp_localize_script(
        ...
        array(
            ...
            'default_button_image_url' => CBFA_PLUGIN_URL . 'assets/icon.png', // <-- Using PNG
...
function cbfa_enqueue_frontend_scripts() {
    ...
    // Retrieve Quick Links, default to empty array if invalid JSON or empty
    $raw_quick_links = get_option('cbfa_quick_links_list', '');
    $quick_links = [];
    if (!empty($raw_quick_links)) {
        // FIX: Add trim() to ensure whitespace doesn't break JSON decoding
        $decoded_links = json_decode(trim($raw_quick_links), true);
        if (is_array($decoded_links)) {
            $quick_links = $decoded_links;
        }
    }
    ...
    $typing_delay = (int) get_option('cbfa_typing_delay_ms_per_char', 25); // Default 25ms/char (fast human typing)
    ...
    wp_localize_script(
        ...
        array_merge(
            array(
                ...
                'quick_links'           => $quick_links, // <-- PASS QUICK LINKS
                'popup_delay'           => $popup_delay,
                'popup_duration'        => $popup_duration,
                'popup_trigger'         => $popup_trigger,
                'typing_delay_ms_per_char' => $typing_delay, 
                'button_image_url'      => get_option('cbfa_button_image_url'), 
                'default_button_image'  => CBFA_PLUGIN_URL . 'assets/icon.png',
