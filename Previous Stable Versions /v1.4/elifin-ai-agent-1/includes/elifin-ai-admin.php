...
function cbfa_output_hidden_preserved_settings($active_tab) {
    ...
    $all_settings_fields = array(
        'configuration' => array('cbfa_api_key', 'cbfa_chatbot_id', 'cbfa_agent_name', 'cbfa_initial_messages', 'cbfa_quick_links_list'),
        ...
        'styling'       => array('cbfa_primary_color', 'cbfa_secondary_color', 'cbfa_agent_name_color', 'cbfa_header_text_color', 'cbfa_agent_message_color', 'cbfa_agent_bubble_text_color', 'cbfa_user_message_color', 'cbfa_user_bubble_text_color', 'cbfa_font_family', 'cbfa_base_font_size', 'cbfa_typing_delay_ms_per_char'),
...
function cbfa_style_section_callback() {
    echo '<p>Set custom colors, fonts, and interaction settings for the chat widget to match your brand.</p>';
}
...
function cbfa_quick_links_list_callback() {
    $default_links = '[{"text": "Track My Order", "link": "/track"}, {"text": "View Discounts", "link": "/offers"}]';
    $option = get_option('cbfa_quick_links_list', $default_links);
    ?>
    <textarea id="cbfa_quick_links_list" name="cbfa_quick_links_list" rows="5" cols="50" class="large-text"><?php echo esc_textarea($option); ?></textarea>
    <p class="description">Enter quick action links in JSON format (e.g., <code>[{"text": "FAQ", "link": "/faq"}, {"text": "Contact Us", "link": "/contact"}]</code>)</p>
    <?php
}
...
function cbfa_image_upload_field_html($key_suffix, $default_url_suffix, $label, $description, $preview_styles) {
    ...
    // Initial background style for button icon if no image is selected
    $primary_color = get_option('cbfa_primary_color', '#4f46e5');
    // Only apply background if it's the button icon and no custom image is set
...
// NEW: Typing Delay Field Callback
function cbfa_typing_delay_ms_per_char_callback() {
    $option = get_option('cbfa_typing_delay_ms_per_char', 25); 
    ?>
    <input type="number" min="0" step="1" id="cbfa_typing_delay_ms_per_char" name="cbfa_typing_delay_ms_per_char" value="<?php echo esc_attr($option); ?>" placeholder="25" class="small-text">
    <p class="description">The delay (in milliseconds) for each character in the agent\'s response to simulate human typing speed. Default is 25ms (fast typing).</p>
    <?php
}
